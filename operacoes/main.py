...
import operacoes.google_client as google_client
import operacoes.database as database
import operacoes.sincronizador_logica as sincronizador_logica
from operacoes.config import PLANILHAS, ARQUIVOS_EXCEL
...
def main():
    print("==== [SYNC] DOMAIN_A ====")
    print("[MAIN] Sincronização DOMAIN_A — Lógica unificada via prefixo")
    try:
        ...
        heavy_sheets = {"housekeeping_audits", "guest_feedback", "extra_revenue"}
        ...
        for item in PLANILHAS:
            ...
            try:
                ...
                cfg = type(
                    "config", (), {
                        "ID_PLANILHA": plan_id,
                        "DOMAIN_A_TAB_NAME": aba_obj.title, # Passa o título da aba que ele encontrou
                        "TABELA_DESTINO": tabela_destino, # Usa o prefix do config como destino
                        "ZERAR_TIMEOUT_COPY": tabela_destino in heavy_sheets,
                    })
        ...
        heavy_excel_files = {"pos_item_sales_2025"}

        for item in ARQUIVOS_EXCEL:
            # A lógica do Excel já está perfeita e não muda.
            try:
                cfg = type("config", (), {
                    "ID_ARQUIVO": item["id"],
                    "TABELA_DESTINO": item["prefix"],
                    "ZERAR_TIMEOUT_COPY": item["prefix"] in heavy_excel_files,
                })
        ...
        print("\n✅ [MAIN] Sincronização DOMAIN_A finalizada!")

    except Exception:
        print("\n[ERRO] Falha geral na sincronização (DOMAIN_A):")
        traceback.print_exc()
